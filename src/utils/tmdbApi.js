...
const tmdbApi = {
  ...
  moviesNowPlaying: region =>
    `${apiBaseUrl}movie/now_playing?${apiKey}&region=${region}`,
  moviesUpcoming: region =>
    `${apiBaseUrl}movie/upcoming?${apiKey}&region=${region}`,
  personDetails: personId => `${apiBaseUrl}person/${personId}?${apiKey}`,
  searchMulti: searchQuery =>
    `${apiBaseUrl}search/multi?${apiKey}&query=${searchQuery}`,
